...
class Video extends Component {
  ...
  getRecipeById = async () => {
    ...
    if (recipes.singleRecipe.msg) {
      ...
      const splitVideo = currentVideo.split("/")[2];
    ...
    return (
      ...
      >
        <div className={video.NextVideoCard}>
          <video
            className={video.VideoImageCard}
            src={isPending ? "" : url + link}
          ></video>{" "}
          <br />
          <span className={video.VideoNextText}>[Step {index}]</span>
          <span className={video.VideoCardTitle}>
            {isPending ? "Loading" : fileName}
          </span>
          <br />
          <span className={video.VideoCardDate}>
            HanaLohana -{" "}
            {isPending ? (
              "loading.."
            ) : (
              <Moment fromNow>{fileDate.toString()}</Moment>
            )}
          </span>
        </div>
      </Link>
          ...
          <Row className={"mt-4"}>
            <Col xs={12} sm={12} md={12} lg={8} xl={8} className={"mb-3"}>
              <video
                ...
                src={isPending ? "" : url + this.state.currentVideo}
                controls
              ></video>{" "}
              <br />
              <span className={video.VideoTitle}>
                {isPending ? "Loading" : this.state.title}
              </span>
              <br />
              <span className={video.VideoDate}>
                {isPending ? (
                  "loading.."
                ) : (
                  <Moment fromNow>{this.state.fileDate}</Moment>
                )}
              </span>
            </Col>
            <Col xs={12} sm={12} md={12} lg={4} xl={4}>
              ...
              {!isPending &&
                this.state.videoRecipe.map((link, index) => {
              ...
              {!isPending &&
              this.state.videoRecipe.length === this.state.indexVideo + 1
                ? "Video Habis"
                : ""}
            </Col>
          </Row>
